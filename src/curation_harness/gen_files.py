...
def build_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument("--include", nargs="+", action="extend", help="filter key=value")
    group.add_argument("--kb", nargs="+", action="extend", help="example: KB5034204")
    ...
    parser.add_argument(
        "--require-proc-info",
        help="only use files with extra info",
        action="store_true",
    )
...
def run(args: argparse.Namespace) -> int:
    if args.count < 1:
        raise SystemExit(f"Error: Count must be 1 or higher. count = {args.count}")
    ...
    if args.include is not None:
        ...
        for key, val in includes:
            ...
            if df.shape[0] == 0:
                raise Exception(f"Query {includes} resulted in empty dateframe. Nothing to process")
            ...
            if not args.regex:
                ...
                if val_type == list:
                    df = df[
                        df[key].apply(
                            lambda x: any(bool(re.search(val, item, re.IGNORECASE)) for item in x)
                        )
                    ]
                else:
        ...
        from kbdex import get_kb_file_urls, get_kb_info, kb_exists
    ...
    for i, file in enumerate(all_files_list):
        bucket.setdefault(i % args.count, []).append(file)
        ...
        if args.limit_list and i / args.count > args.limit_list - 1:
    ...
    if args.include is not None:
        ...
        paths_values = df["Path"].apply(lambda x: PureWindowsPath(x).parent).value_counts()

        with pd.option_context("display.max_rows", None, "display.max_columns", None):
            print(sources_values)
            print(paths_values)

        (meta_path / "sources_values.md").write_text(sources_values.to_markdown())
        (meta_path / "paths_values.md").write_text(paths_values.to_markdown())
